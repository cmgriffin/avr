...
void SPI_init(const SPI_Init_Typedef *init)
{
    ...
    SPCR |=
        ...
        (init->clkSelect) |
        (init->interuptEn
             ? SPI_SPCR_INTERUPT_EN
             : 0) |
        (init->dataOrderLsb ? SPI_SPCR_DATA_ORDER_LSB
                            : 0) |
        (init->masterSelect ? SPI_SPCR_MASTER_MODE
                            : 0) |
        (init->clkPolHigh ? SPI_SPCR_CLK_POL
                          : 0) |
        (init->clkPhaseTrailing ? SPI_SPCR_CLK_PH
                                : 0);
