...
SD_RETURN_CODES sd_init(void)
{
    ...
    // can operate in supplied voltage range. Check whether the card is first
    // generation or Version 2.00 (or later). If the card is of first
    // generation, it will respond with R1 with bit 2 set (illegal command)
    ...
    if (SD_Response[0] == 0x01)
    {
        ...
        // Continue to send ACMD41 (always preceded by CMD55) until the card
        // responds with 'in_idle_state', which is R1 = 0x00.
    ...
    // Read and write operations are performed on SD cards in blocks of set
...
uint8_t sd_write_single_block(uint32_t addr, uint8_t *buf)
{
    ...
    if (res1 == 0)
    {
        ...
        {
            if ((res1 = SPI_transferByte(0xff)) != 0xFF)
                break;
        }
...
uint8_t sd_read_single_block(uint32_t addr, uint8_t *buf)
{
    ...
    if (res1 != 0xFF)
    {
        ...
        // The host should use 100ms timeout (minimum) for single and multiple
        // read operations
        readAttempts = 0;
        while (++readAttempts != SD_MAX_READ_ATTEMPTS)
        {
            if ((read = SPI_transferByte(0xff)) != 0xFF)
                break;
        }
...
static uint8_t sd_read_response1(void)
{
    ...
    while ((response = SPI_transferByte(0xff)) == 0xFF)
    {
        i++;
...
static void sd_read_response3_7(uint8_t *res)
{
    ...
    if (res[0] > 1)
        return; // if error reading R1, return

    res[1] = SPI_transferByte(0xff);
    res[2] = SPI_transferByte(0xff);
    res[3] = SPI_transferByte(0xff);
    res[4] = SPI_transferByte(0xff);
}

static void sd_command(uint8_t cmd, uint32_t arg, uint8_t crc)
{
    // Transmit command
    SPI_transferByte(cmd | 0x40);

    // Transmit argument
    SPI_transferByte((uint8_t)(arg >> 24));
    SPI_transferByte((uint8_t)(arg >> 16));
    SPI_transferByte((uint8_t)(arg >> 8));
    SPI_transferByte((uint8_t)(arg));

    // Transmit CRC
    SPI_transferByte(crc | 0x01);
}
