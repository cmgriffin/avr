...
static inline uint8_t __I2C_getStatus()
{
...
void I2C_init(I2C_Init_Typedef *init)
{
    ...
    TWSR |= init->clkSelect;
    TWCR |= I2C_TWCR_TWI_EN |
            (init->interuptEn ? I2C_TWCR_INTERUPT_EN : 0);
    GPIO_setValueHigh(&((GPIO_TypeDef)GPIO_PC4)); // set SDA pullup
    GPIO_setValueHigh(&((GPIO_TypeDef)GPIO_PC5)); // set SDA pullup
...
void I2C_setBitRate(uint16_t bitRateKhz)
{
    ...
    _I2C_DEBUG("set twbr=%d, clkSel=%d for bitRate=%d", twbr, clkSel, bitRateKhz);
}
